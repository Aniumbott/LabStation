...
// TooltipProvider import removed for now as part of diagnosis.
...
type ResourceTypeSortableColumn = 'name' | 'resourceCount';
          ...
          {activeContextId === GLOBAL_CONTEXT_VALUE && (
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              <div className="lg:col-span-2 space-y-6">
                <Card>
                  <CardHeader className="flex flex-row items-center justify-between">
                    <div><CardTitle className="text-xl flex items-center gap-2"><Building className="h-5 w-5 text-primary"/>All Labs ({filteredLabsWithCounts.length})</CardTitle><CardDescription>View, add, or manage operations for individual labs.</CardDescription></div>
                    <div className="flex gap-2">
                        <FilterSortDialog open={isLabFilterDialogOpen} onOpenChange={setIsLabFilterDialogOpen}><FilterSortDialogTrigger asChild><Button variant="outline" size="xs" className="h-7"><FilterIcon />Filter</Button></FilterSortDialogTrigger><FilterSortDialogContent className="sm:max-w-md"><FilterSortDialogHeader><FilterSortDialogTitle>Filter &amp; Sort Labs</FilterSortDialogTitle></FilterSortDialogHeader><Separator className="my-3" /><div className="space-y-3"><div className="relative"><Label htmlFor="labSearchDialog">Search (Name/Loc/Desc)</Label><SearchIcon className="absolute left-2.5 top-[calc(1.25rem_+_8px)] h-4 w-4 text-muted-foreground" /><Input id="labSearchDialog" value={tempLabSearchTerm} onChange={e => setTempLabSearchTerm(e.target.value)} placeholder="Keyword..." className="mt-1 h-9 pl-8"/></div><div><Label htmlFor="labSortDialog">Sort by</Label><Select value={tempLabSortBy} onValueChange={setTempLabSortBy}><SelectTrigger id="labSortDialog" className="mt-1 h-9"><SelectValue /></SelectTrigger><SelectContent>{labSortOptions.map(opt => <SelectItem key={opt.value} value={opt.value}>{opt.label}</SelectItem>)}</SelectContent></Select></div></div><FilterSortDialogFooter className="mt-4 pt-4 border-t"><Button variant="ghost" onClick={resetLabDialogFiltersOnly} className="mr-auto"><FilterX />Reset</Button><Button variant="outline" onClick={() => setIsLabFilterDialogOpen(false)}><X />Cancel</Button><Button onClick={handleApplyLabDialogFilters}><CheckCircle2 />Apply</Button></FilterSortDialogFooter></FilterSortDialogContent></FilterSortDialog>
                        <Button onClick={handleOpenNewLabDialog} size="sm" className="h-7"><PlusCircle />Lab</Button>
                    </div>
                  </CardHeader>
                  <CardContent className="p-0">
                    {filteredLabsWithCounts.length > 0 ? (<ScrollArea className="max-h-[400px]"><div className="overflow-x-auto border-t"><Table><TableHeader><TableRow><TableHead>Name</TableHead><TableHead>Location</TableHead><TableHead className="text-center">Resources</TableHead><TableHead className="text-center">Members</TableHead><TableHead className="text-right">Actions</TableHead></TableRow></TableHeader><TableBody>{filteredLabsWithCounts.map(lab => (<TableRow key={lab.id}><TableCell className="font-medium">{lab.name}</TableCell><TableCell>{lab.location || 'N/A'}</TableCell><TableCell className="text-center">{lab.resourceCount}</TableCell><TableCell className="text-center">{lab.memberCount}</TableCell><TableCell className="text-right space-x-1"><Button variant="outline" size="icon" className="h-8 w-8" onClick={() => handleOpenEditLabDialog(lab)}><Edit /></Button><Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => setActiveContextId(lab.id)}><Settings2 className="text-primary"/></Button><AlertDialog open={labToDelete?.id === lab.id} onOpenChange={(isOpen) => !isOpen && setLabToDelete(null)}><AlertDialogTrigger asChild><Button variant="ghost" size="icon" className="text-destructive hover:bg-destructive/10 hover:text-destructive h-8 w-8" onClick={() => setLabToDelete(lab)}><Trash2 /></Button></AlertDialogTrigger><AlertDialogContent><AlertDialogHeader><AlertDialogTitle>Delete "{labToDelete?.name}"?</AlertDialogTitle><AlertDialogDescription>Ensure no resources or active memberships are assigned.</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel>Cancel</AlertDialogCancel><AlertDialogAction variant="destructive" onClick={() => labToDelete && handleDeleteLab(labToDelete.id)}>Delete</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog></TableCell></TableRow>))}</TableBody></Table></div></ScrollArea>) : (<p className="p-6 text-center text-muted-foreground">No labs defined yet.</p>)}
                  </CardContent>
                </Card>
                <Card>
                  <CardHeader className="flex flex-row items-center justify-between">
                    <div><CardTitle className="text-xl flex items-center gap-2"><Wrench className="h-5 w-5 text-primary"/>All Maintenance Requests ({filteredMaintenanceRequests.length})</CardTitle></div>
                    <div className="flex gap-2">
                        <FilterSortDialog open={isMaintenanceFilterDialogOpen} onOpenChange={setIsMaintenanceFilterDialogOpen}><FilterSortDialogTrigger asChild><Button variant="outline" size="xs" className="h-7"><FilterIcon/>Filter</Button></FilterSortDialogTrigger><FilterSortDialogContent className="w-full max-w-lg"><FilterSortDialogHeader><FilterSortDialogTitle>Filter All Maintenance Requests</FilterSortDialogTitle></FilterSortDialogHeader><Separator className="my-3" /><div className="space-y-3"><div className="relative"><Label htmlFor="maintSearchSystemAll">Search (Resource/Reporter/Issue/Tech)</Label><SearchIcon className="absolute left-2.5 top-[calc(1.25rem_+_8px)] h-4 w-4 text-muted-foreground" /><Input id="maintSearchSystemAll" value={tempMaintenanceSearchTerm} onChange={e => setTempMaintenanceSearchTerm(e.target.value)} placeholder="Keyword..." className="mt-1 h-9 pl-8"/></div><div className="grid grid-cols-1 sm:grid-cols-2 gap-4"><div><Label htmlFor="maintStatusSystemAll">Status</Label><Select value={tempMaintenanceFilterStatus} onValueChange={(v) => setTempMaintenanceFilterStatus(v as MaintenanceRequestStatus | 'all')}><SelectTrigger id="maintStatusSystemAll" className="h-9 mt-1"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">All Statuses</SelectItem>{maintenanceRequestStatuses.map(s => <SelectItem key={s} value={s}>{s}</SelectItem>)}</SelectContent></Select></div><div><Label htmlFor="maintResourceSystemAll">Resource</Label><Select value={tempMaintenanceFilterResourceId} onValueChange={setTempMaintenanceFilterResourceId} disabled={allResourcesForCountsAndChecks.length === 0}><SelectTrigger id="maintResourceSystemAll" className="h-9 mt-1"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">All Resources</SelectItem>{allResourcesForCountsAndChecks.map(r => <SelectItem key={r.id} value={r.id}>{r.name}</SelectItem>)}</SelectContent></Select></div></div><div className="grid grid-cols-1 sm:grid-cols-2 gap-4"><div><Label htmlFor="maintTechSystemAll">Assigned Technician</Label><Select value={tempMaintenanceFilterTechnicianId} onValueChange={setTempMaintenanceFilterTechnicianId} disabled={allTechniciansForMaintenance.length === 0}><SelectTrigger id="maintTechSystemAll" className="h-9 mt-1"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">All/Any</SelectItem><SelectItem value="--unassigned--">Unassigned</SelectItem>{allTechniciansForMaintenance.map(t => <SelectItem key={t.id} value={t.id}>{t.name}</SelectItem>)}</SelectContent></Select></div><div><Label htmlFor="maintLabSystemAll">Lab (of Resource)</Label><Select value={tempMaintenanceFilterLabId} onValueChange={setTempMaintenanceFilterLabId} disabled={labs.length === 0}><SelectTrigger id="maintLabSystemAll" className="mt-1 h-9"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">All Labs</SelectItem><SelectItem value="--global--">Resources not in a specific Lab</SelectItem>{labs.map(lab => (<SelectItem key={lab.id} value={lab.id}>{lab.name}</SelectItem>))}</SelectContent></Select></div></div></div><FilterSortDialogFooter className="mt-4 pt-4 border-t"><Button variant="ghost" onClick={resetMaintenanceDialogFiltersOnly} className="mr-auto"><FilterX />Reset</Button><Button variant="outline" onClick={() => setIsMaintenanceFilterDialogOpen(false)}><X />Cancel</Button><Button onClick={handleApplyMaintenanceDialogFilters}><CheckCircle2 />Apply</Button></FilterSortDialogFooter></FilterSortDialogContent></FilterSortDialog>
                        <Button onClick={handleOpenNewMaintenanceDialog} size="sm" className="h-7"><PlusCircle />Log Request</Button>
                    </div>
                  </CardHeader>
                  <CardContent className="p-0">
                      <ScrollArea className="max-h-96 border-t">
                      {filteredMaintenanceRequests.length > 0 ? (<Table><TableHeader><TableRow><TableHead>Resource</TableHead><TableHead className="min-w-[150px]">Issue</TableHead><TableHead>Reported</TableHead><TableHead>Status</TableHead><TableHead>Assigned</TableHead><TableHead className="text-right"></TableHead></TableRow></TableHeader><TableBody>{filteredMaintenanceRequests.map((request) => (<TableRow key={request.id}><TableCell className="font-medium">{request.resourceName}</TableCell><TableCell className="text-xs text-muted-foreground max-w-[150px] truncate" title={request.issueDescription}>{request.issueDescription}</TableCell><TableCell className="text-xs">{request.reportedByUserName}</TableCell><TableCell>{getMaintenanceStatusBadge(request.status)}</TableCell><TableCell className="text-xs">{request.assignedTechnicianName || <span className="italic">Unassigned</span>}</TableCell><TableCell className="text-right p-1"><Button variant="outline" size="icon" className="h-7 w-7" onClick={() => handleOpenEditMaintenanceDialog(request)}><Edit /></Button></TableCell></TableRow>))}</TableBody></Table>) : <p className="text-center text-sm text-muted-foreground py-6">{activeMaintenanceFilterCount > 0 ? "No requests match filters." : "No maintenance requests."}</p>}
                      </ScrollArea>
                  </CardContent>
                </Card>
                <Card>
                  <CardHeader className="flex flex-row items-center justify-between">
                    <div><CardTitle className="text-xl flex items-center gap-2"><ListChecks className="h-5 w-5 text-primary"/>Resource Types ({filteredResourceTypesWithCount.length})</CardTitle><CardDescription>Define categories for lab resources.</CardDescription></div>
                     <div className="flex gap-2">
                        <FilterSortDialog open={isResourceTypeFilterDialogOpen} onOpenChange={setIsResourceTypeFilterDialogOpen}><FilterSortDialogTrigger asChild><Button variant="outline" size="xs" className="h-7"><FilterIcon />Filter</Button></FilterSortDialogTrigger><FilterSortDialogContent className="sm:max-w-md"><FilterSortDialogHeader><FilterSortDialogTitle>Filter &amp; Sort Resource Types</FilterSortDialogTitle></FilterSortDialogHeader><Separator className="my-3" /><div className="space-y-3"><div className="relative"><Label htmlFor="typeSearchDialog">Search (Name)</Label><SearchIcon className="absolute left-2.5 top-[calc(1.25rem_+_8px)] h-4 w-4 text-muted-foreground" /><Input id="typeSearchDialog" value={tempResourceTypeSearchTerm} onChange={e => setTempResourceTypeSearchTerm(e.target.value)} placeholder="Keyword..." className="mt-1 h-9 pl-8"/></div><div><Label htmlFor="typeSortDialog">Sort by</Label><Select value={tempResourceTypeSortBy} onValueChange={setTempResourceTypeSortBy}><SelectTrigger id="typeSortDialog" className="mt-1 h-9"><SelectValue /></SelectTrigger><SelectContent>{resourceTypeSortOptions.map(opt => <SelectItem key={opt.value} value={opt.value}>{opt.label}</SelectItem>)}</SelectContent></Select></div></div><FilterSortDialogFooter className="mt-4 pt-4 border-t"><Button variant="ghost" onClick={resetResourceTypeDialogFiltersOnly} className="mr-auto"><FilterX />Reset</Button><Button variant="outline" onClick={() => setIsResourceTypeFilterDialogOpen(false)}><X />Cancel</Button><Button onClick={handleApplyResourceTypeDialogFilters}><CheckCircle2 />Apply</Button></FilterSortDialogFooter></FilterSortDialogContent></FilterSortDialog>
                        <Button onClick={handleOpenNewResourceTypeDialog} size="sm" className="h-7"><PlusCircle />Type</Button>
                    </div>
                  </CardHeader>
                  <CardContent className="p-0">
                    {filteredResourceTypesWithCount.length > 0 ? (<ScrollArea className="max-h-96"><div className="overflow-x-auto border-t"><Table><TableHeader><TableRow><TableHead>Name</TableHead><TableHead className="text-center"># Resources</TableHead><TableHead className="text-right w-[100px]">Actions</TableHead></TableRow></TableHeader><TableBody>{filteredResourceTypesWithCount.map(type => (<TableRow key={type.id}><TableCell className="font-medium">{type.name}</TableCell><TableCell className="text-center">{type.resourceCount}</TableCell><TableCell className="text-right space-x-1"><Button variant="outline" size="icon" className="h-8 w-8" onClick={() => handleOpenEditResourceTypeDialog(type)}><Edit /></Button><AlertDialog open={typeToDelete?.id === type.id} onOpenChange={(isOpen) => !isOpen && setTypeToDelete(null)}><AlertDialogTrigger asChild><Button variant="ghost" size="icon" className="text-destructive hover:bg-destructive/10 hover:text-destructive h-8 w-8" onClick={() => setTypeToDelete(type)}><Trash2 /></Button></AlertDialogTrigger><AlertDialogContent><AlertDialogHeader><AlertDialogTitle>Delete "{typeToDelete?.name}"?</AlertDialogTitle><AlertDialogDescription>Ensure no resources use this type (must be 0).</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel>Cancel</AlertDialogCancel><AlertDialogAction variant="destructive" onClick={() => typeToDelete && handleDeleteResourceType(typeToDelete.id)}>Delete</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog></TableCell></TableRow>))}</TableBody></Table></div></ScrollArea>) : (<p className="p-6 text-center text-muted-foreground">No resource types defined.</p>)}
                  </CardContent>
                </Card>
              </div>
              ...
              <div className="lg:col-span-1 space-y-6">
                  ...
                  <CardContent className="pt-0">
                    <Tabs defaultValue={activeGlobalClosuresTab} onValueChange={setActiveGlobalClosuresTab} className="text-sm">
                      ...
                      <TabsContent value="specific-dates-global" className="space-y-2">
                        ...
                        {filteredBlackoutDates.length > 0 ? (<Table><TableHeader><TableRow><TableHead className="text-xs p-2">Date</TableHead><TableHead className="text-xs p-2">Reason</TableHead><TableHead className="text-right p-2"></TableHead></TableRow></TableHeader><TableBody>{filteredBlackoutDates.map(bd => (<TableRow key={bd.id}><TableCell className="text-xs p-2">{formatDateSafe(parseISO(bd.date), 'N/A', 'MMM dd, yyyy')}</TableCell><TableCell className="text-xs p-2 truncate max-w-[100px]" title={bd.reason}>{bd.reason || 'N/A'}</TableCell><TableCell className="text-right p-1 space-x-0.5"><Button variant="outline" size="icon" className="h-7 w-7" onClick={() => handleOpenEditDateDialog(bd)}><Edit /></Button><AlertDialog open={dateToDelete?.id === bd.id} onOpenChange={(isOpen) => !isOpen && setDateToDelete(null)}><AlertDialogTrigger asChild><Button variant="ghost" size="icon" className="h-7 w-7 text-destructive hover:bg-destructive/10" onClick={() => setDateToDelete(bd)}><Trash2 /></Button></AlertDialogTrigger><AlertDialogContent><AlertDialogHeader><AlertDialogTitle>Delete Global Blackout?</AlertDialogTitle></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel>Cancel</AlertDialogCancel><AlertDialogAction variant="destructive" onClick={() => dateToDelete && handleDeleteBlackoutDate(dateToDelete.id)}>Delete</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog></TableCell></TableRow>))}</TableBody></Table>) : <p className="text-center text-xs text-muted-foreground py-3">No global specific dates.</p>}
                        </ScrollArea>
                      </TabsContent>
                      <TabsContent value="recurring-rules-global" className="space-y-2">
                        <ScrollArea className="max-h-60 border rounded-md">
                        {filteredRecurringRules.length > 0 ? (<Table><TableHeader><TableRow><TableHead className="text-xs p-2">Rule</TableHead><TableHead className="text-xs p-2">Days</TableHead><TableHead className="text-right p-2"></TableHead></TableRow></TableHeader><TableBody>{filteredRecurringRules.map(rule => (<TableRow key={rule.id}><TableCell className="text-xs p-2 font-medium truncate max-w-[100px]" title={rule.name}>{rule.name}</TableCell><TableCell className="text-xs p-2 truncate max-w-[100px]" title={rule.daysOfWeek.join(', ')}>{rule.daysOfWeek.join(', ')}</TableCell><TableCell className="text-right p-1 space-x-0.5"><Button variant="outline" size="icon" className="h-7 w-7" onClick={() => handleOpenEditRecurringDialog(rule)}><Edit /></Button><AlertDialog open={ruleToDelete?.id === rule.id} onOpenChange={(isOpen) => !isOpen && setRuleToDelete(null)}><AlertDialogTrigger asChild><Button variant="ghost" size="icon" className="h-7 w-7 text-destructive hover:bg-destructive/10" onClick={() => setRuleToDelete(rule)}><Trash2 /></Button></AlertDialogTrigger><AlertDialogContent><AlertDialogHeader><AlertDialogTitle>Delete Global Rule?</AlertDialogTitle></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel>Cancel</AlertDialogCancel><AlertDialogAction variant="destructive" onClick={() => ruleToDelete && handleDeleteRecurringRule(ruleToDelete.id)}>Delete</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog></TableCell></TableRow>))}</TableBody></Table>) : <p className="text-center text-xs text-muted-foreground py-3">No global recurring rules.</p>}
                        </ScrollArea>
                      </TabsContent>
                    </Tabs>
                  </CardContent>
                ...
                <Card>
                    <CardHeader className="flex flex-row items-center justify-between">
                        <div><CardTitle className="text-xl flex items-center gap-2"><Users className="h-5 w-5 text-primary"/>All Pending Lab Access Requests ({filteredLabAccessRequests.length})</CardTitle></div>
                        <FilterSortDialog open={isSystemWideAccessRequestsFilterOpen} onOpenChange={setIsSystemWideAccessRequestsFilterOpen}>
                            <FilterSortDialogTrigger asChild><Button variant="outline" size="xs" className="h-7"><FilterIcon />Filter</Button></FilterSortDialogTrigger>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="lg:col-span-2 space-y-6">
                   <Card>
                    <CardHeader className="flex flex-row items-start justify-between">
                        <div><CardTitle className="text-xl flex items-center gap-2"><Users className="h-5 w-5 text-primary"/>Members &amp; Access for {selectedLabDetails.name}</CardTitle></div>
                        <Button size="sm" className="h-7" onClick={() => setIsManualAddMemberDialogOpen(true)}><Users />Member</Button>
                    </CardHeader>
                    <CardContent className="pt-0">
                  ...
                  <Card>
                    <CardHeader className="flex flex-row items-center justify-between">
                        <div><CardTitle className="text-xl flex items-center gap-2"><Wrench className="h-5 w-5 text-primary"/>Maintenance for {selectedLabDetails.name}</CardTitle></div>
                         <div className="flex gap-2">
                            <FilterSortDialog open={isMaintenanceFilterDialogOpen} onOpenChange={setIsMaintenanceFilterDialogOpen}><FilterSortDialogTrigger asChild><Button variant="outline" size="xs" className="h-7"><FilterIcon />Filter</Button></FilterSortDialogTrigger><FilterSortDialogContent className="w-full max-w-lg"><FilterSortDialogHeader><FilterSortDialogTitle>Filter Maintenance for {selectedLabDetails.name}</FilterSortDialogTitle></FilterSortDialogHeader><Separator className="my-3" /><div className="space-y-3"><div className="relative"><Label htmlFor="maintSearchLab">Search (Resource/Reporter/Issue/Tech)</Label><SearchIcon className="absolute left-2.5 top-[calc(1.25rem_+_8px)] h-4 w-4 text-muted-foreground" /><Input id="maintSearchLab" value={tempMaintenanceSearchTerm} onChange={e => setTempMaintenanceSearchTerm(e.target.value)} placeholder="Keyword..." className="mt-1 h-9 pl-8"/></div><div className="grid grid-cols-1 sm:grid-cols-2 gap-4"><div><Label htmlFor="maintStatusLab">Status</Label><Select value={tempMaintenanceFilterStatus} onValueChange={(v) => setTempMaintenanceFilterStatus(v as MaintenanceRequestStatus | 'all')}><SelectTrigger id="maintStatusLab" className="h-9 mt-1"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">All Statuses</SelectItem>{maintenanceRequestStatuses.map(s => <SelectItem key={s} value={s}>{s}</SelectItem>)}</SelectContent></Select></div><div><Label htmlFor="maintResourceLab">Resource</Label><Select value={tempMaintenanceFilterResourceId} onValueChange={setTempMaintenanceFilterResourceId} disabled={resourcesInSelectedLab.length === 0}><SelectTrigger id="maintResourceLab" className="h-9 mt-1"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">All Resources</SelectItem>{resourcesInSelectedLab.map(r => <SelectItem key={r.id} value={r.id}>{r.name}</SelectItem>)}</SelectContent></Select></div></div><div><Label htmlFor="maintTechLab">Assigned Technician</Label><Select value={tempMaintenanceFilterTechnicianId} onValueChange={setTempMaintenanceFilterTechnicianId} disabled={allTechniciansForMaintenance.length === 0}><SelectTrigger id="maintTechLab" className="h-9 mt-1"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">All/Any</SelectItem><SelectItem value="--unassigned--">Unassigned</SelectItem>{allTechniciansForMaintenance.map(t => <SelectItem key={t.id} value={t.id}>{t.name}</SelectItem>)}</SelectContent></Select></div></div><FilterSortDialogFooter className="mt-4 pt-4 border-t"><Button variant="ghost" onClick={resetMaintenanceDialogFiltersOnly} className="mr-auto"><FilterX />Reset</Button><Button variant="outline" onClick={() => setIsMaintenanceFilterDialogOpen(false)}><X />Cancel</Button><Button onClick={handleApplyMaintenanceDialogFilters}><CheckCircle2 />Apply</Button></FilterSortDialogFooter></FilterSortDialogContent></FilterSortDialog>
                            <Button onClick={handleOpenNewMaintenanceDialog} size="sm" className="h-7"><PlusCircle />Log Request</Button>
                         </div>
                    </CardHeader>
                    <CardContent className="p-0">
                        <ScrollArea className="max-h-96 border-t">
                        {filteredMaintenanceRequests.length > 0 ? (<Table><TableHeader><TableRow><TableHead className="p-2 text-xs">Resource</TableHead><TableHead className="p-2 text-xs">Issue</TableHead><TableHead className="p-2 text-xs">Status</TableHead><TableHead className="text-right p-2 text-xs"></TableHead></TableRow></TableHeader><TableBody>{filteredMaintenanceRequests.map(req => (<TableRow key={req.id}><TableCell className="p-2 text-xs font-medium">{req.resourceName}</TableCell><TableCell className="p-2 text-xs truncate max-w-[100px]" title={req.issueDescription}>{req.issueDescription}</TableCell><TableCell className="p-2">{getMaintenanceStatusBadge(req.status)}</TableCell><TableCell className="text-right p-1"><Button variant="outline" size="icon" className="h-7 w-7" onClick={() => handleOpenEditMaintenanceDialog(req)}><Edit /></Button></TableCell></TableRow>))}</TableBody></Table>) : <p className="text-center text-sm text-muted-foreground py-6">No maintenance requests for this lab.</p>}
                        </ScrollArea>
                    </CardContent>
                  </Card>
                </div>
                ...
                <div className="lg:col-span-1 space-y-6">
                  <Card>
                    ...
                    <CardContent className="space-y-2 text-sm">
                        <div className="flex justify-between items-center py-1 border-b border-dashed"><span>Resources:</span><span className="font-bold text-primary">{resourcesInSelectedLab.length}</span></div>
                        <div className="flex justify-between items-center py-1 border-b border-dashed"><span>Active Members:</span> <span className="font-bold text-primary">{activeLabMembers.length}</span></div>
                        <div className="flex justify-between items-center py-1"><span>Open Maintenance:</span> <span className="font-bold text-primary">{maintenanceForSelectedLab.filter(mr => mr.status === 'Open' || mr.status === 'In Progress').length}</span></div>
                    </CardContent>
                  </Card>
                  <Card>
                    <CardHeader className="flex flex-row items-center justify-between">
                    ...
                    <CardContent className="pt-0">
                        ...
                        <TabsContent value="specific-dates-lab" className="space-y-2">
                            <ScrollArea className="max-h-60 border rounded-md">
                            {filteredBlackoutDates.length > 0 ? (<Table><TableHeader><TableRow><TableHead className="text-xs p-2">Date</TableHead><TableHead className="text-xs p-2">Reason</TableHead><TableHead className="text-right p-2"></TableHead></TableRow></TableHeader><TableBody>{filteredBlackoutDates.map(bd => (<TableRow key={bd.id}><TableCell className="text-xs p-2">{formatDateSafe(parseISO(bd.date), 'N/A', 'MMM dd, yyyy')}</TableCell><TableCell className="text-xs p-2 truncate max-w-[100px]" title={bd.reason}>{bd.reason || 'N/A'}</TableCell><TableCell className="text-right p-1 space-x-0.5"><Button variant="outline" size="icon" className="h-7 w-7" onClick={() => handleOpenEditDateDialog(bd)}><Edit /></Button><AlertDialog open={dateToDelete?.id === bd.id} onOpenChange={(isOpen) => !isOpen && setDateToDelete(null)}><AlertDialogTrigger asChild><Button variant="ghost" size="icon" className="h-7 w-7 text-destructive hover:bg-destructive/10" onClick={() => setDateToDelete(bd)}><Trash2 /></Button></AlertDialogTrigger><AlertDialogContent><AlertDialogHeader><AlertDialogTitle>Delete Blackout for {selectedLabDetails.name}?</AlertDialogTitle></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel>Cancel</AlertDialogCancel><AlertDialogAction variant="destructive" onClick={() => dateToDelete && handleDeleteBlackoutDate(dateToDelete.id)}>Delete</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog></TableCell></TableRow>))}</TableBody></Table>) : <p className="text-center text-xs text-muted-foreground py-3">No specific dates.</p>}
                            </ScrollArea>
                        </TabsContent>
                        <TabsContent value="recurring-rules-lab" className="space-y-2">
                            <ScrollArea className="max-h-60 border rounded-md">
                            {filteredRecurringRules.length > 0 ? (<Table><TableHeader><TableRow><TableHead className="text-xs p-2">Rule</TableHead><TableHead className="text-xs p-2">Days</TableHead><TableHead className="text-right p-2"></TableHead></TableRow></TableHeader><TableBody>{filteredRecurringRules.map(rule => (<TableRow key={rule.id}><TableCell className="text-xs p-2 font-medium truncate max-w-[100px]" title={rule.name}>{rule.name}</TableCell><TableCell className="text-xs p-2 truncate max-w-[100px]" title={rule.daysOfWeek.join(', ')}>{rule.daysOfWeek.join(', ')}</TableCell><TableCell className="text-right p-1 space-x-0.5"><Button variant="outline" size="icon" className="h-7 w-7" onClick={() => handleOpenEditRecurringDialog(rule)}><Edit /></Button><AlertDialog open={ruleToDelete?.id === rule.id} onOpenChange={(isOpen) => !isOpen && setRuleToDelete(null)}><AlertDialogTrigger asChild><Button variant="ghost" size="icon" className="h-7 w-7 text-destructive hover:bg-destructive/10" onClick={() => setRuleToDelete(rule)}><Trash2 /></Button></AlertDialogTrigger><AlertDialogContent><AlertDialogHeader><AlertDialogTitle>Delete Rule for {selectedLabDetails.name}?</AlertDialogTitle></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel>Cancel</AlertDialogCancel><AlertDialogAction variant="destructive" onClick={() => ruleToDelete && handleDeleteRecurringRule(ruleToDelete.id)}>Delete</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog></TableCell></TableRow>))}</TableBody></Table>) : <p className="text-center text-xs text-muted-foreground py-3">No recurring rules.</p>}
                            </ScrollArea>
                        </TabsContent>
                        </Tabs>
                    </CardContent>
