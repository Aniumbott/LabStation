...
const getMaintenanceStatusBadge = (status: MaintenanceRequestStatus) => {
  switch (status) {
    ...
    case 'Resolved': return <Badge className="bg-blue-500 text-white border-transparent"><LucideCheckCircle className="mr-1 h-3.5 w-3.5" />{status}</Badge>; // Explicitly use LucideCheckCircle
    case 'Closed': return <Badge className="bg-green-500 text-white border-transparent"><LucideCheckCircle className="mr-1 h-3.5 w-3.5" />{status}</Badge>; // Explicitly use LucideCheckCircle
...
export default function LabOperationsCenterPage() {
  ...
  const [closureContextLabId, setClosureContextLabId] = useState<string>('__global__'); // '__global__' for Global
  ...
  const [tempMaintenanceFilterLabId, setTempMaintenanceFilterLabId] = useState<string>('all'); // New filter
  ...
  const [activeMaintenanceFilterLabId, setActiveMaintenanceFilterLabId] = useState<string>('all'); // New filter
  ...
  const fetchAllAdminData = useCallback(async () => {
    ...
    try {
      ...
      setBlackoutDates(boSnapshot.docs.map(d => ({ id: d.id, ...d.data() } as BlackoutDate))); // labId is already optional
      const rrQuery = query(collection(db, "recurringBlackoutRules"), orderBy("name", "asc"));
      const rrSnapshot = await getDocs(rrQuery);
      setRecurringRules(rrSnapshot.docs.map(d => ({ id: d.id, ...d.data() } as RecurringBlackoutRule))); // labId is already optional
  ...
  const filteredBlackoutDates = useMemo(() => {
    return blackoutDates.filter(bd => {
        const labMatch = closureContextLabId === '__global__' ? !bd.labId : bd.labId === closureContextLabId;
        return labMatch;
    });
  }, [blackoutDates, closureContextLabId]);
  const filteredRecurringRules = useMemo(() => {
    return recurringRules.filter(rule => {
        const labMatch = closureContextLabId === '__global__' ? !rule.labId : rule.labId === closureContextLabId;
        return labMatch;
    });
  }, [recurringRules, closureContextLabId]);
  ...
  const filteredLabAccessRequests = useMemo(() => {
    return labAccessRequests.filter(req => {
        return activeLabAccessFilterLabId === 'all' || req.labId === activeLabAccessFilterLabId;
    });
  }, [labAccessRequests, activeLabAccessFilterLabId]);
  const handleApplyLabAccessDialogFilters = useCallback(() => { setActiveLabAccessFilterLabId(tempLabAccessFilterLabId); setIsLabAccessFilterDialogOpen(false); }, [tempLabAccessFilterLabId]);
  const resetLabAccessDialogFiltersOnly = useCallback(() => { setTempLabAccessFilterLabId('all'); }, []);
  const resetAllActiveLabAccessPageFilters = useCallback(() => { setActiveLabAccessFilterLabId('all'); resetLabAccessDialogFiltersOnly(); setIsLabAccessFilterDialogOpen(false); }, [resetLabAccessDialogFiltersOnly]);
  const activeLabAccessFilterCount = useMemo(() => [activeLabAccessFilterLabId !== 'all'].filter(Boolean).length, [activeLabAccessFilterLabId]);
          ...
          <TabsContent value="lab-access-requests" className="mt-6">
            <Card>
              <CardHeader className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
                  <div><CardTitle>Lab Access Requests</CardTitle><CardDescription>Review and manage pending requests for lab access.</CardDescription></div>
                  <div className="flex gap-2 flex-wrap">
                    <FilterSortDialog open={isLabAccessFilterDialogOpen} onOpenChange={setIsLabAccessFilterDialogOpen}>
                        <FilterSortDialogTrigger asChild><Button variant="outline" size="sm"><ListFilter className="mr-2 h-4 w-4" />Filter by Lab {activeLabAccessFilterCount > 0 && (<Badge variant="secondary" className="ml-1 rounded-full px-1.5 text-xs">{activeLabAccessFilterCount}</Badge>)}</Button></FilterSortDialogTrigger>
                        <FilterSortDialogContent className="w-full max-w-md">
                            <FilterSortDialogHeader><FilterSortDialogTitle>Filter Lab Access Requests</FilterSortDialogTitle></FilterSortDialogHeader>
                            <Separator className="my-3" />
                            <div className="space-y-3">
                                <div><Label htmlFor="labAccessFilterLabDialog">Lab Requested</Label><Select value={tempLabAccessFilterLabId} onValueChange={setTempLabAccessFilterLabId} disabled={labs.length === 0}><SelectTrigger id="labAccessFilterLabDialog" className="mt-1 h-9"><SelectValue placeholder="Filter by Lab"/></SelectTrigger><SelectContent><SelectItem value="all">All Labs</SelectItem>{labs.map(lab => (<SelectItem key={lab.id} value={lab.id}>{lab.name}</SelectItem>))}</SelectContent></Select></div>
                            </div>
                            <FilterSortDialogFooter className="mt-4 pt-4 border-t"><Button variant="ghost" onClick={resetLabAccessDialogFiltersOnly} className="mr-auto"><FilterX className="mr-2 h-4 w-4"/>Reset</Button><Button variant="outline" onClick={() => setIsLabAccessFilterDialogOpen(false)}><X className="mr-2 h-4 w-4"/>Cancel</Button><Button onClick={handleApplyLabAccessDialogFilters}><CheckCircle2 className="mr-2 h-4 w-4"/>Apply</Button></FilterSortDialogFooter>
              ...
              <CardContent className="p-0">
                {isLabAccessRequestLoading ? (
                  ...
                  <div className="overflow-x-auto border rounded-md shadow-sm">
                    <Table>
                      <TableHeader><TableRow><TableHead>User</TableHead><TableHead>Lab Requested</TableHead><TableHead>Date Requested</TableHead><TableHead className="text-right">Actions</TableHead></TableRow></TableHeader>
                      <TableBody>
                  ...
                  <div className="text-center py-10 text-muted-foreground">
                    ...
                    <p className="font-medium">{activeLabAccessFilterCount > 0 ? "No requests match filters." : "No pending lab access requests."}</p>
                    {activeLabAccessFilterCount > 0 && <Button variant="link" onClick={resetAllActiveLabAccessPageFilters} className="mt-2 text-xs"><FilterX className="mr-1.5 h-3.5 w-3.5"/>Reset Filters</Button>}
                  </div>
                )}
              </CardContent>
            </Card>
