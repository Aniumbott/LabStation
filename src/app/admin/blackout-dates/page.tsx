...
export default function BlackoutDatesPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      setBlackoutDates(boSnapshot.docs.map(d => ({ id: d.id, ...d.data() } as BlackoutDate))); // date is YYYY-MM-DD string
  ...
  const filteredBlackoutDates = useMemo(() => {
    return blackoutDates.filter(bd => {
        ...
        return !activeDateSearchTerm ||
               ...
               (bd.date && format(parseISO(bd.date), 'PPP').toLowerCase().includes(lowerSearchTerm)); // date is 'YYYY-MM-DD' string
    });
  ...
  const handleSaveBlackoutDate = useCallback(async (data: BlackoutDateFormValues) => {
    ...
    const formattedDateOnly = format(data.date, 'yyyy-MM-dd'); // data.date is JS Date from form
    const displayDate = format(data.date, 'PPP');

    const blackoutDataToSave = {
      date: formattedDateOnly,
      reason: data.reason || null,
    };
    // Remove null fields before saving
    Object.keys(blackoutDataToSave).forEach(key => {
      if ((blackoutDataToSave as any)[key] === null) {
        delete (blackoutDataToSave as any)[key];
      }
    });
    ...
    try {
      if (editingBlackoutDate) {
        ...
        addAuditLog(currentUser.id, currentUser.name || 'Admin', 'BLACKOUT_DATE_UPDATED', { entityType: 'BlackoutDate', entityId: editingBlackoutDate.id, details: `Blackout Date for ${displayDate} updated. Reason: ${data.reason || 'N/A'}`});
        ...
        addAuditLog(currentUser.id, currentUser.name || 'Admin', 'BLACKOUT_DATE_CREATED', { entityType: 'BlackoutDate', entityId: docRef.id, details: `Blackout Date for ${displayDate} created. Reason: ${data.reason || 'N/A'}`});
  ...
  const handleDeleteBlackoutDate = useCallback(async (blackoutDateId: string) => {
    ...
    try {
      ...
      addAuditLog(currentUser.id, currentUser.name || 'Admin', 'BLACKOUT_DATE_DELETED', { entityType: 'BlackoutDate', entityId: blackoutDateId, details: `Blackout Date for ${format(parseISO(deletedDateObj.date), 'PPP')} (Reason: ${deletedDateObj.reason || 'N/A'}) deleted.`});
  ...
  const resetDateDialogFilters = useCallback(() => {
  ...
  const handleSaveRecurringRule = useCallback(async (data: RecurringBlackoutRuleFormValues) => {
    ...
    const ruleDataToSave = {
     ...
     Object.keys(ruleDataToSave).forEach(key => {
      if ((ruleDataToSave as any)[key] === null) {
        delete (ruleDataToSave as any)[key];
      }
    });
    ...
    try {
      if (editingRecurringRule) {
        ...
        addAuditLog(currentUser.id, currentUser.name || 'Admin', 'RECURRING_RULE_UPDATED', { entityType: 'RecurringBlackoutRule', entityId: editingRecurringRule.id, details: `Recurring rule '${data.name}' updated.`});
        ...
        addAuditLog(currentUser.id, currentUser.name || 'Admin', 'RECURRING_RULE_CREATED', { entityType: 'RecurringBlackoutRule', entityId: docRef.id, details: `Recurring rule '${data.name}' created.`});
  ...
  const handleDeleteRecurringRule = useCallback(async (ruleId: string) => {
    ...
    try {
      ...
      await deleteDoc(docRef);
      addAuditLog(currentUser.id, currentUser.name || 'Admin', 'RECURRING_RULE_DELETED', { entityType: 'RecurringBlackoutRule', entityId: ruleId, details: `Recurring rule '${deletedRuleObj.name}' deleted.`});
  ...
  return (
    <TooltipProvider>
                ...
                <CardContent>
                  ...
                  {activeDateFilterCount > 0 ? (
                    <Button variant="outline" size="sm" onClick={resetAllActiveDatePageFilters}>
                     ...
                     !isLoading && blackoutDates.length === 0 && (
                      <Button onClick={handleOpenNewDateDialog} size="sm">
      ...
      {canManageBlackouts && ( // Only render dialogs if user has permission
        <>
