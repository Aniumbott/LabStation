...
import { Archive, ListChecks, PackagePlus, Edit, Trash2, Filter as FilterIcon, FilterX, Search as SearchIcon, Loader2, X, CheckCircle2, Building, PlusCircle, CalendarOff, Repeat, Wrench, ListFilter, PenToolIcon, AlertCircle, CheckCircle, Globe, Users, ThumbsUp, ThumbsDown } from 'lucide-react';
...
export default function LabManagementPage() {
  ...
  const [currentTab, setCurrentTab] = useState<string>("labs");
  ...
  const [tempClosureLabIdFilter, setTempClosureLabIdFilter] = useState<string>('all');
  const [activeClosureLabIdFilter, setActiveClosureLabIdFilter] = useState<string>('all');
  ...
  const fetchAllAdminData = useCallback(async () => {
    ...
    try {
      // Labs, Resource Types, and all Resources data
      ...
      setBlackoutDates(boSnapshot.docs.map(d => ({ id: d.id, ...d.data(), labId: d.data().labId || null } as BlackoutDate)));
      const rrQuery = query(collection(db, "recurringBlackoutRules"), orderBy("name", "asc"));
      const rrSnapshot = await getDocs(rrQuery);
      setRecurringRules(rrSnapshot.docs.map(d => ({ id: d.id, ...d.data(), labId: d.data().labId || null } as RecurringBlackoutRule)));

      // All Users (for Maintenance and Lab Access Requests)
      ...
      setAllUsersForMaintenance(fetchedUsersAll); // Used by Maintenance and Lab Access Requests
      ...
      const pendingRequestsPromises = labAccessSnapshot.docs.map(async (memberDoc) => {
          ...
          return {
              ...membershipData,
              id: memberDoc.id, // ensure doc ID is part of the object
  ...
  const handleLabAccessRequestAction = async (
    ...
    try {
      ...
      if (result.success) {
        ...
        fetchAllAdminData(); // Refresh all data, including access requests
      } else {
  ...
  // --- Blackout Dates & Recurring Rules Logic ---
  useEffect(() => { if (isDateFilterDialogOpen) { setTempDateSearchTerm(activeDateSearchTerm); setTempClosureLabIdFilter(activeClosureLabIdFilter); }}, [isDateFilterDialogOpen, activeDateSearchTerm, activeClosureLabIdFilter]);

  const filteredBlackoutDates = useMemo(() => {
    return blackoutDates.filter(bd => {
        const labMatch = activeClosureLabIdFilter === 'all' || bd.labId === activeClosureLabIdFilter || (activeClosureLabIdFilter === '--global--' && !bd.labId);
        ...
        return labMatch && (!activeDateSearchTerm || reasonMatch || dateMatch);
    });
  }, [blackoutDates, activeDateSearchTerm, activeClosureLabIdFilter]);

  const filteredRecurringRules = useMemo(() => {
    return recurringRules.filter(rule => {
        const labMatch = activeClosureLabIdFilter === 'all' || rule.labId === activeClosureLabIdFilter || (activeClosureLabIdFilter === '--global--' && !rule.labId);
        const lowerSearchTerm = activeDateSearchTerm.toLowerCase();
        ...
        return labMatch && (!activeDateSearchTerm || nameMatch || reasonMatch);
    });
  }, [recurringRules, activeDateSearchTerm, activeClosureLabIdFilter]);
  ...
  const handleApplyDateDialogFilters = useCallback(() => { setActiveDateSearchTerm(tempDateSearchTerm); setActiveClosureLabIdFilter(tempClosureLabIdFilter); setIsDateFilterDialogOpen(false); }, [tempDateSearchTerm, tempClosureLabIdFilter]);
  const resetDateDialogFiltersOnly = useCallback(() => { setTempDateSearchTerm(''); setTempClosureLabIdFilter('all'); }, []);
  const resetAllActiveDatePageFilters = useCallback(() => { setActiveDateSearchTerm(''); setActiveClosureLabIdFilter('all'); resetDateDialogFiltersOnly(); setIsDateFilterDialogOpen(false); }, [resetDateDialogFiltersOnly]);
  const activeDateFilterCount = useMemo(() => [activeDateSearchTerm !== '', activeClosureLabIdFilter !== 'all'].filter(Boolean).length, [activeDateSearchTerm, activeClosureLabIdFilter]);
  ...
  return (
    <TooltipProvider>
      <div className="space-y-8">
        <PageHeader title="Lab Management" description="Define labs, resource types, closures, manage maintenance, and lab access requests." icon={Archive} />
              ...
              <CardContent className="p-0">
                {isLabAccessRequestLoading ? (
                            ...
                            <TableCell className="text-right space-x-1">
                               <Tooltip>
                                <TooltipTrigger asChild>
                                    <Button variant="outline" size="icon" className="h-8 w-8" onClick={() => handleLabAccessRequestAction(req.id!, req.userId, req.userName!, req.labId, req.labName!, 'approve_request')} disabled={isProcessingLabAccessRequest[req.id!]}>
                                        {isProcessingLabAccessRequest[req.id!] && actionBeingProcessed === 'approve' ? <Loader2 className="h-4 w-4 animate-spin"/> : <ThumbsUp className="h-4 w-4 text-green-600" />}
                               ...
                               <Tooltip>
                                <TooltipTrigger asChild>
                                    <Button variant="destructive" size="icon" className="h-8 w-8" onClick={() => handleLabAccessRequestAction(req.id!, req.userId, req.userName!, req.labId, req.labName!, 'reject_request')} disabled={isProcessingLabAccessRequest[req.id!]}>
                                         {isProcessingLabAccessRequest[req.id!] && actionBeingProcessed === 'reject' ? <Loader2 className="h-4 w-4 animate-spin"/> : <ThumbsDown className="h-4 w-4" />}
                  ...
                  <div className="text-center py-10 text-muted-foreground">
                    ...
                    <p className="font-medium">No pending lab access requests.</p>
                  </div>
                )}
              </CardContent>
            </Card>
      ...
      {isDateFormDialogOpen && currentUser && (<BlackoutDateFormDialog open={isDateFormDialogOpen} onOpenChange={setIsDateFormDialogOpen} initialBlackoutDate={editingBlackoutDate} onSave={handleSaveBlackoutDate} labs={labs} />)}
      {isRecurringFormDialogOpen && currentUser && (<RecurringBlackoutRuleFormDialog open={isRecurringFormDialogOpen} onOpenChange={setIsRecurringFormDialogOpen} initialRule={editingRecurringRule} onSave={handleSaveRecurringRule} labs={labs} />)}
