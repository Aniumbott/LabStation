...
interface ManageUnavailabilityDialogProps {
  ...
  onSaveUnavailability: (updatedPeriods: UnavailabilityPeriod[]) => void;
}
...
export function ManageUnavailabilityDialog({ resource, open, onOpenChange, onSaveUnavailability }: ManageUnavailabilityDialogProps) {
  ...
  const handleAddPeriod = () => {
    ...
    if (isBefore(selectedDateRange.to, selectedDateRange.from)) {
  ...
  const handleSaveChanges = () => {
    onSaveUnavailability(currentPeriods);
    onOpenChange(false);
  };
                ...
                <div>
                  ...
                  />
                   <p className="text-xs text-muted-foreground mt-1">
                    Selected: {selectedDateRange?.from ? format(selectedDateRange.from, 'PPP') : 'None'} - {selectedDateRange?.to ? format(selectedDateRange.to, 'PPP') : 'None'}
                  </p>
                </div>
                <div>
                      ...
                      {currentPeriods.map((period) => (
                        <TableRow key={period.id}>
                          ...
                          <TableCell className="text-right">
                            <Button variant="ghost" size="icon" className="h-8 w-8 text-destructive hover:text-destructive" onClick={() => handleDeletePeriod(period.id)}>
